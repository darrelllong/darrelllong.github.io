...
The third repository in this set is [`secret-sharing`](https://github.com/darrelllong/secret-sharing). The [cryptography crate](https://github.com/darrelllong/cryptography) covers ciphers, hashes, and public-key primitives. The [entropy harness](/blog/2026-05-05-auditing-random-number-generators) audits the random sources those primitives depend on. This one is an in-tree, pure-Rust implementation of every constructive *threshold secret-sharing* scheme from the foundational literature — including several that mainstream libraries omit.
